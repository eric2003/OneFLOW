...
void DumpCsvFile( const std::string & filename, vec1d & x, std::vector<std::vector<double>> & u );
...
void numerical( int nx, int ns, int nt, double dx, double dt )
{
    ...
    vec1d x;
    x.Allocate( 0, nx, 0 ); // numerical solsution at every time step

    vec1d un;
    un.Allocate( 0, nx, 0 ); // numerical solsution at every time step

    vec1d ut;
    ut.Allocate( 0, nx, 0 ); // temporary array during RK3 integration
