...
{
    std::string csvname = std::format( "field_final{}.csv", Global::iter+1 );
    this->DumpField( csvname, grid->x, u );
}

void WenoField::DumpField( const std::string & filename, Vec1d & x, Vec1d & u )
{
    std::fstream file;
    ...
    {
        std::string str = {};
        str += std::format( "{:.16f} {:.16f}", x[ i ], u[ i ] );
...
{
    runge_kutta_stage1( N, un, u, res, dx, dt );
    runge_kutta_stage2( N, un, u, res, dx, dt );
    runge_kutta_stage3( N, un, u, res, dx, dt );
    un = u;
}
...
void runge_kutta_stage3( int N, Vec1d & un, Vec1d & u, Vec1d & res, double dx, double dt )
{
    ...
    for ( int i = 0; i < N; ++ i )
    {
        u[ i ] = ( 1.0 / 3.0 ) * un[ i ] + ( 2.0 / 3.0 ) * u[ i ] + ( 2.0 / 3.0 ) * dt * res[ i ];
    }
    boundary( N, u );
}
