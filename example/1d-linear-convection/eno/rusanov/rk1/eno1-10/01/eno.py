...
def calc_eno_coef( iorder, coef ):
    for r in range(-1,iorder):
        for j in range(iorder):
            coef[r+1, j] = calculate_eno_crj(r, j, iorder)
...
def visualize(eno):
    ...
    #计算理论解
    u_analytical = analytical_solution(eno.mesh.xcc, eno.T, eno.c, eno.L)
...
def plot_eno_OrderAnalysis(solver, xcc, u_list, u_analytical):
    ...
    plt.figure("OneFLOW-CFD Solver", figsize=(10, 6))
    plt.title(f'1D Convection Equation at t = {solver.T:.3f} using [1-{n}]th-order ENO and {solver.rk}{ordinal}-order Runge-Kutta methods')
    for i in range(0, n):
    ...
    plt.plot(xcc, u_analytical, 'r--', linewidth=0.5, markersize=5, marker='o', markerfacecolor='none',label='Analytical')
    plt.xlabel('x')
    plt.ylabel('u')
    plt.legend()
    plt.grid(True, color='gray', linestyle='--', linewidth=0.5, alpha=0.7)    
...
def RunEno(solver, mesh, iorder):
    ...
    calc_eno_coef(eno.iorder, eno.coef)
    init_field(eno)
...
def performEnoOrderAnalysis(iorder_max):
    mesh = Mesh()
    solver = Solver()
    #计算理论解
    ...
    solver.rk = 3
    #solver.dt = 0.025/64
    solver.dt = 0.025/(np.power(2, 9))
    for iorder in range(1, iorder_max+1):
        u = RunEno(solver, mesh, iorder)
        print(f'iorder={iorder},NOrder={iorder_max},T={solver.T:.3f},dt={solver.dt},dx={mesh.dx},nt={int(solver.T/solver.dt)},runge_kutta{solver.rk},ENO{iorder},{fluxnames[solver.iflux]} FLUX')
        u_list.append(u)
...
def main():
    #performEnoOrderAnalysis(20)
    performEnoOrderAnalysis(12)
    #performEnoTimestepAnalysis()
