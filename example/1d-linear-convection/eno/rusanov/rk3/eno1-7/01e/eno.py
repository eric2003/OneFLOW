...
def plot_eno_field(xcc_list, u_list):
    # 定义一个包含不同颜色、线形和标记的列表
    styles = [
    ...
    for i in range(0, n):
        ...
        plt.plot(xcc_list[i], u_list[i], marker=style['marker'], markerfacecolor='none', linestyle=style['linestyle'], color=style['color'], \
                markersize=5, linewidth=0.5, alpha=1.0, label=f'Numerical (Rusanov)')
    plt.xlabel('x')
    plt.ylabel('u')
    plt.legend()
    plt.grid(True, color='gray', linestyle='--', linewidth=0.5, alpha=0.7)    
...
def RunEno(iorder, mesh):
    ...
    print(t)
    #visualize(eno)
    return eno.xcc, np.copy(eno.u[eno.ist:eno.ied+1])
...
def main():
    ...
    for iorder in range(1, iorder_max+1):
        xcc, u = RunEno(mesh, iorder)
        xcc_list.append(xcc)
        u_list.append(u)
        
    plot_eno_field(xcc_list, u_list)
