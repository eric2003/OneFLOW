...
def plot_eno_TimestepAnalysis(solver, xcc, u_list, u_analytical, dt_list, iorder):
    ...
    plt.figure("OneFLOW-CFD Solver", figsize=(10, 6))
    plt.title(f'1D Convection Equation at t = {solver.T:.3f} using [iorder]th-order ENO and {solver.rk}{ordinal}-order Runge-Kutta methods')
    for i in range(0, n):
...
def performEnoTimestepAnalysis():
    ...
    for i in range(0, n):
        ...
        print(f'T={solver.T:.3f},dt={solver.dt},nt={int(solver.T/solver.dt)}')
        solver.dt /= 2
        
    plot_eno_TimestepAnalysis(solver, mesh.xcc, u_list, u_analytical, dt_list, iorder)
    
def main():
    #performEnoOrderAnalysis()
