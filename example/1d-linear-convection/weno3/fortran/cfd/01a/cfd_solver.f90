...
module cfd_solver
    ...
    ! Abstract reconstructor base class with explicit interface
    ...
    ! Define abstract interface BEFORE using it
    abstract interface
        subroutine reconstruct_interface(this, q, cfd)
            import :: ReconstructorType, dp
            class(ReconstructorType), intent(inout) :: this
            real(dp), intent(in) :: q(:)
            class(*), intent(inout) :: cfd
...
contains
    ...
    ! ENO reconstruction
    subroutine eno_reconstruct(this, q, cfd_obj)
        class(EnoReconstructorType), intent(inout) :: this
        real(dp), intent(in) :: q(:)
        class(*), intent(inout) :: cfd_obj
        
        type(CfdType), pointer :: cfd
        ...
        type is (CfdType)
            cfd => cfd_obj
        class default
            error stop "Invalid CFD object type in eno_reconstruct"
        end select
    ...
    ! WENO reconstruction
    subroutine weno_reconstruct(this, q, cfd_obj)
        ...
        type is (CfdType)
            cfd => cfd_obj
        class default
            error stop "Invalid CFD object type in weno_reconstruct"
        end select
