...
contains
    ...
    subroutine weno3L_periodic(cfd, ist, ied, nghosts, u, qL)
    ...
    subroutine weno3R_periodic(cfd, ist, ied, nghosts, u, qR)
        type(CfdType), intent(in) :: cfd
        real(dp), intent(in) :: u(:)
    ...
    subroutine reconstruction(q, cfd)
        ...
        if (trim(cfd%config%recon_scheme) == "eno") then
			...
			call cfd%eno_recon%reconstruct(q, cfd, cfd%domain, cfd%solution)
