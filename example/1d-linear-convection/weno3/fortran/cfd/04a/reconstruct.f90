...
module recon_module
    ...
    private
	public :: init_coef, eno_reconstruct, weno_reconstruct, reconstruction
...
contains
    ...
    subroutine recon_reconstruct(this, q, config, domain, solution)
        ...
        if (trimmed_scheme == "eno") then
			call eno_reconstruct(this%eno_recon, q, domain, solution)
        else if (trimmed_scheme == "weno") then
            call weno_reconstruct(this%weno_recon, q, domain, solution)
        else
            error stop "Unknown reconstruction scheme in reconstruction(): "//trimmed_scheme
        end if
