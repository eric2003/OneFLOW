...
contains
    ...
    subroutine eno_init(this, spatial_order, ntcells)
	  ...
	  interface
		subroutine my_init_coef(spatial_order, coef)
		  import :: dp
		  integer, intent(in) :: spatial_order
		  real(dp), intent(out) :: coef(:,:)
		end subroutine my_init_coef
	  end interface		
		...
		call my_init_coef(spatial_order, this%coef)
    ...
    subroutine reconstruction(q, cfd)
        ...
        ! 原代码：select type(rec => cfd%reconstructor)
        ! 新代码：用if判断调用对应重构器的方法
        if (trim(cfd%config%recon_scheme) == "eno") then
            ! 检查ENO重构器是否已分配
            if (.not. allocated(cfd%eno_recon)) then
                error stop "ENO reconstructor not initialized in reconstruction!"
            end if
            call cfd%eno_recon%reconstruct(q, cfd)
        else if (trim(cfd%config%recon_scheme) == "weno") then
            ! 检查WENO重构器是否已分配
            if (.not. allocated(cfd%weno_recon)) then
                error stop "WENO reconstructor not initialized in reconstruction!"
            end if
            call cfd%weno_recon%reconstruct(q, cfd)
        else
