...
module cfd_solver
	...
	use recon_module, only: init_coef, EnoReconstructorType, WenoReconstructorType, &
  ...
  !  end subroutine weno3R_periodic
    ...
    subroutine reconstruction(q, cfd)
        ...
        if (trim(cfd%config%recon_scheme) == "eno") then
			call eno_reconstruct(cfd%eno_recon, q, cfd%domain, cfd%solution)
        else if (trim(cfd%config%recon_scheme) == "weno") then
            call weno_reconstruct(cfd%weno_recon, q, cfd%domain, cfd%solution)
        else
