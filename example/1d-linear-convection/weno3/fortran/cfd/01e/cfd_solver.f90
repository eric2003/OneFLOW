...
contains
    ...
    ! ENO reconstruction - SIMPLIFIED VERSION
    subroutine first_order_reconstruct(this, q, cfd)
        ...
        do i = ist, ied
            ...
            !cfd%solution%q_face_left(j) = 0.5_dp * (q(i-1) + q(i))
            !cfd%solution%q_face_right(j) = 0.5_dp * (q(i) + q(i+1))
            ! Simple averaging for testing
    ...
    subroutine performEnoWenoAnalysis()
        ...
        config_eno3%spatial_order = 2  ! Use 2nd order for stability
        config_eno3%flux_type = 0
        config_eno3%rk_order = 1
        config_eno3%wave_speed = 1.0_dp
        config_eno3%final_time = 0.625_dp
        config_eno3%cfl = 0.5_dp
        config_eno3%dt = 0.001_dp  ! Small time step
        ...
        config_weno3%cfl = 0.3_dp  ! More strict CFL for WENO
        config_weno3%dt = 0.0005_dp  ! Even smaller time step
