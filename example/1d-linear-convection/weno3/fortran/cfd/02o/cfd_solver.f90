...
module cfd_solver
	...
	use recon_module, only: init_coef, ReconType, EnoReconstructorType, WenoReconstructorType, &
  ...
  !  end subroutine cfd_init
    ...
    subroutine cfd_init(this, config, domain)
        ...
        if (trim(config%recon_scheme) == "eno") then
            ! 分配并初始化ENO重构器
            allocate(this%my_recon%eno_recon)
            call this%my_recon%eno_recon%init(config%spatial_order, domain%ntcells)
        else if (trim(config%recon_scheme) == "weno") then
            ! 分配WENO重构器
            allocate(this%my_recon%weno_recon)
        else
            error stop "Unknown reconstruction scheme: "//trim(config%recon_scheme)
        end if
    ...
    subroutine reconstruction(q, cfd)
        ...
        if (trim(cfd%config%recon_scheme) == "eno") then
			call eno_reconstruct(cfd%my_recon%eno_recon, q, cfd%domain, cfd%solution)
        else if (trim(cfd%config%recon_scheme) == "weno") then
            call weno_reconstruct(cfd%my_recon%weno_recon, q, cfd%domain, cfd%solution)
        else
