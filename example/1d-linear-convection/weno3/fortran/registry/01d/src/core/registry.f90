...
module registry_module
    ...
    public :: register_component_simple, register_component_with_factory
    public :: create_component, initialize_registry, cleanup_registry
...
contains
    ...
    ! Registration with factory procedure
    ...
    subroutine create_component(category, name, instance)
        ...
        if (len_trim(info%category) == 0) then
            print *, "[ERROR] Component not found: ", trim(cat_lower), ".", trim(name_lower)
            return
        end if
        
        if (.not. info%has_factory) then
            print *, "[ERROR] Component has no factory: ", trim(cat_lower), ".", trim(name_lower)
            return
        end if
        
        if (.not. associated(info%factory)) then
            print *, "[ERROR] Factory procedure not associated: ", trim(cat_lower), ".", trim(name_lower)
            return
        end if
    ...
    subroutine ci_create(this, instance)
        ...
        if (.not. this%has_factory) then
            error stop "[ERROR] Component has no factory procedure"
        end if
        
        if (.not. associated(this%factory)) then
            error stop "[ERROR] Factory procedure not associated"
        end if
