...
contains
    ...
    subroutine solver_base_run_to_time(this, final_time)
        ...
        if (this%config%verbose) then
            print *, "[SOLVER] Running from t = ", this%current_time, &
                     " to t = ", final_time
        end if
        
        do while (this%current_time < final_time)
            ! 计算时间步长
            dt = min(this%config%dt, final_time - this%current_time)
            ...
            ! 每10步输出一次进度
            if (mod(step_count, 10) == 0 .and. this%config%verbose) then
                print *, "[SOLVER] Progress: t = ", this%current_time, &
                         " / ", final_time
            end if
        end do
