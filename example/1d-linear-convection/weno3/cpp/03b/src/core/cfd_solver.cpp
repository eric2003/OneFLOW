...
void CfdSolver::initialize(const CfdConfig& config) {
    ...
    // Create initial condition using factory - 完全委托给工厂
    initial_condition_ = InitialConditionFactory::create_from_config(config_);
    
    // Create residual calculator (使用新的字符串flux_type)
    residual_calculator_ = ResidualCalculatorFactory::create_from_config(config_, true);
    
    // Create time integrator (使用新的time_integration)
    time_integrator_ = TimeIntegratorFactory::create_integrator(
        config_.time_integration,
        std::make_unique<ConvectionResidualCalculator>(
            ReconstructorFactory::create_reconstructor(config_),
            FluxFactory::create_flux_calculator(config_.flux_type),  // 使用字符串
    ...
    // 如果是θ方法，设置theta参数
    if (config_.time_integration.find("theta") != std::string::npos) {
        auto theta_integrator = dynamic_cast<ThetaMethod*>(time_integrator_.get());
        if (theta_integrator) {
            // theta参数已在配置中，TimeIntegratorFactory会自动设置
        }
    }	
    ...
    if (config_.verbose) {
        ...
        if (config_.is_implicit()) {
            std::cout << "  Implicit method: Yes" << std::endl;
            std::cout << "  Max CFL (theoretical): ∞" << std::endl;
        }
    }
}
...
Real CfdSolver::analytical_solution(Real x, Real t) const {
    ...
    Real xmin = domain_.mesh().xmin();
    Real xmax = domain_.mesh().xmax();
    Real L = xmax - xmin;

    if (L <= 0.0) {
        // 无效域，直接使用当前初始条件
        return initial_condition_->evaluate(x - a * t);
    }

    // 计算平移后的位置
    Real x_shifted = x - a * t;

    // 周期性边界处理：将坐标映射到 [xmin, xmax) 区间
    // 1. 计算相对于xmin的位置
    Real relative_pos = x_shifted - xmin;

    // 2. 取模运算，确保在 [0, L) 范围内
    relative_pos = std::fmod(relative_pos, L);

    // 3. 处理fmod的负数结果（fmod(-0.1, 2.0) = -0.1）
    if (relative_pos < 0.0) {
        relative_pos += L;
    }

    // 4. 转换回绝对坐标
    x_shifted = xmin + relative_pos;

    // 5. 处理边界浮点误差
    constexpr Real EPS = 1.0e-12;
    if (x_shifted > xmax - EPS) {
        x_shifted = xmin;
    }
...
Vector CfdSolver::run_single_simulation(const CfdConfig& config) {
    ...
    return solver.run_simulation(config.final_time);
}
