...
void CfdSolver::residual(const Vector& q) {
    ...
    Vector q_temp = q;
    domain_.apply_periodic_boundary(q_temp);
...
void CfdSolver::perform_eno_weno_analysis() {
    ...
    config_eno.output_dir = ".";  // 写入当前目录
    config_weno.output_dir = "."; // 写入当前目录
    ...
    const auto& xcc = solver.get_domain().mesh().cell_centers();
    for (int i = 0; i < solver.get_domain().mesh().ncells(); ++i) {
        eno_file << std::setw(12) << xcc[i] 
                << std::setw(12) << u_eno[i] << "\n";
    }
    eno_file.close();
    ...
    for (int i = 0; i < solver.get_domain().mesh().ncells(); ++i) {
        analytical_file << std::setw(12) << xcc[i] 
                       << std::setw(12) << u_analytical[i] << "\n";
    }
    analytical_file.close();
    
    // Also copy to results directory
    std::filesystem::path results_dir("results");
    if (!std::filesystem::exists(results_dir)) {
        std::filesystem::create_directories(results_dir);
    }
    
    std::filesystem::copy_file("eno_results.txt", results_dir / "eno_results.txt",
                              std::filesystem::copy_options::overwrite_existing);
    std::filesystem::copy_file("weno_results.txt", results_dir / "weno_results.txt",
                              std::filesystem::copy_options::overwrite_existing);
    std::filesystem::copy_file("analytical_results.txt", results_dir / "analytical_results.txt",
                              std::filesystem::copy_options::overwrite_existing);
    ...
    std::cout << "  (also copied to results/ directory)" << std::endl;
