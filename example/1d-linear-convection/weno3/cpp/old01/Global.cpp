...
GoverningEquation Global::governing_equation;
int Global::istart = 0;
int Global::iviscous = 0;
int Global::iconservation = 1;
int Global::nsave = -1;
int Global::idump_initial_field = -1;
int Global::ifinite_volume = 1;
double Global::total_time = 0.0;
double Global::dt = 0.1;
int Global::nt = -1;
int Global::iter = -1;
int Global::iter_start = 0;
int Global::cell_dim = -1;
int Global::phys_dim = -1;
int Global::nghost = -1;
int Global::nequ = -1;
std::string Global::file_string="";
...
void Scheme::read( const json & j )
{
    this->set_inviscid_scheme( j[ "inviscid" ] );
    this->set_reconstruction_scheme( j[ "reconstruction" ] );
    this->set_viscous_scheme( j[ "viscous" ] );
    this->set_time_scheme( j[ "time" ] );
}

void Scheme::set_inviscid_scheme( const std::string & name )
{
    if ( name == "center" )
    {
        this->inviscid = to_int( BasicScheme::CENTER );
    }
    else if ( name == "ftcs" )
    {
        this->inviscid = to_int( BasicScheme::CENTER );
    ...
    {
        this->inviscid = to_int( BasicScheme::HLLC );
    }
    else if ( name == "lax" )
    {
        this->inviscid = to_int( BasicScheme::LAX );
    }
    else if ( name == "roe" )
    {
        this->inviscid = to_int( BasicScheme::Roe );
    }
    else if ( name == "rusanov" )
    {
        this->inviscid = to_int( BasicScheme::Rusanov );
    }
    else if ( name == "upwind1" )
    {
        this->inviscid = to_int( BasicScheme::UpWind1 );
    }
    else if ( name == "upwind2" )
    {
        this->inviscid = to_int( BasicScheme::UpWind2 );
    }
    else if ( name == "weno5" )
    {
        this->inviscid = to_int( BasicScheme::WENO );
    }
    else if ( name == "crweno5" )
    {
        this->inviscid = to_int( BasicScheme::CRWENO );
    }
    else
    {
        this->inviscid = to_int( BasicScheme::WENO );
...
void Scheme::set_reconstruction_scheme( const std::string & name )
{
    if ( name == "weno5" )
    {
        this->reconstruction = to_int( BasicScheme::WENO );
    }
    else if ( name == "crweno5" )
    {
        this->reconstruction = to_int( BasicScheme::CRWENO );
    }
    else if ( name == "upwind1" )
    {
        this->reconstruction = to_int( BasicScheme::UpWind1 );
    }
    else if ( name == "upwind2" )
    {
        this->reconstruction = to_int( BasicScheme::UpWind2 );
    }
}

void Scheme::set_viscous_scheme( const std::string & name )
{
    if ( name == "center" )
    {
        this->viscous = to_int( BasicScheme::CENTER );
    }
    else
    {
        this->viscous = to_int( BasicScheme::CENTER );
...
void Scheme::set_time_scheme( const std::string & name )
{
    if ( name == "cn" )
    {
        this->time_scheme = to_int( BasicScheme::CN );
    }
    else if ( name == "icp" )
    {
        this->time_scheme = to_int( BasicScheme::ICP );
    }
    else if ( name == "rk1" )
    {
        this->time_scheme = to_int( BasicScheme::RK1 );
    }
    else if ( name == "rk2" )
    {
        this->time_scheme = to_int( BasicScheme::RK2 );
    }
    else if ( name == "rk3" )
    {
        this->time_scheme = to_int( BasicScheme::RK3 );
    }
    else if ( name == "ftcs" )
    {
        this->time_scheme = to_int( BasicScheme::RK1 );
    ...
    for ( int i = ist; i <= ied; ++ i )
    {
        ...
        if ( i == 1 )
        {
            ...
            for ( int ig = Global::ifinite_volume; ig <= Global::nghost; ++ ig )
            {
                ...
                ijk_ghosts.push_back( ic - ig );
                ijk_donors.push_back( ic + iig );
            }
        }
        else
        {
            //ig = 0: interface value
            //ig=1,2,... ghost cell value
            int ic = i - Global::ifinite_volume;
            for ( int ig = Global::ifinite_volume; ig <= Global::nghost; ++ ig )
            {
                int iig = ig - Global::ifinite_volume;
                ijk_ghosts.push_back( ic + ig );
...
void Interface::SendGeom( int zone, std::vector<int> & donorfaces )
{
    ...
    int ndata = sub_donorijk.size() * Global::nequ;

    std::vector<double> sub_donordata( ndata );
