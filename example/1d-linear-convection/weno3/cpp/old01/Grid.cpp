...
void Grid::Allocate( int nNodes )
{
    this->ni = nNodes;
    this->nic = nNodes - 1; //ncells
    this->x.Allocate( 0, nNodes - 1 );
    this->xcc.Allocate( 0, this->nic - 1 ); //cell center;
}

void Grid::CalcMetrics()
{
    for ( int i = 0; i < nic; ++ i )
    {
        xcc[ i ] = 0.5 * ( x[ i ] + x[ i + 1 ] );
