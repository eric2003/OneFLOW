...
class CFDAutomation:
    ...
    def _find_executable(self):
        """查找生成的可执行文件（改进版：自动搜索整个 build 目录）"""
        ...
        # 如果没找到特征名，再 fallback 到旧硬编码路径（兼容性）
        search_paths = []
        exe_names = ["OneFLOW_CFD.exe", "oneflow_cfd.exe", "OneFLOW_CFD_Example.exe"]

        for exe_name in exe_names:
            if sys.platform == "win32":
                search_paths.extend([
                    self.build_dir / "bin" / self.build_type / exe_name,
                    self.build_dir / "bin" / exe_name,
                    self.build_dir / self.build_type / exe_name,
                ])
        ...
        for path in search_paths:
            if path.exists():
                self.executable = path
                self.bin_dir = path.parent
                print(f"✓ Found executable (fallback): {self.executable}")
                return

        print("⚠ Could not find executable automatically")
        if candidates:
            print(" Found other .exe files:", [str(p) for p in candidates])
        print(" Tip: Check CMAKE_RUNTIME_OUTPUT_DIRECTORY in CMakeLists.txt")
