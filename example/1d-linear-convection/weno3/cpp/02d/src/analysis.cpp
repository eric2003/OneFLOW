...
void Analysis::run_eno_weno(int cells, double final_time, bool verbose) {
    ...
    config_eno.spatial_order = 3;
    config_eno.flux_type = 0;
    config_eno.rk_order = 2;
    ...
    // Write results - 根据初始条件类型使用不同的文件名
    //std::string suffix = (ic_type == "complex") ? "_complex" : "";
    std::string suffix = "";
    ...
    summary << ic_type << " ENO vs WENO Analysis Summary" << std::endl;
    summary << "==========================================" << std::endl;
