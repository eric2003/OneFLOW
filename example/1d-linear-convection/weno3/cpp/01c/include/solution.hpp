...
public:
    ...
    void initialize_field(const std::function<Real(Real)>& init_func) {
        ...
        for (int i = 0; i < mesh.ncells(); ++i) {
            u_[domain_.ist() + i] = init_func(xcc[i]);
        }
        
        // Apply boundary conditions
        apply_boundary();
        
        // Initialize old field
        copy_to_old();
    }
    
    // Initialize with step function (default)
    void initialize_step_function(Real low_value = 1.0, 
                                  Real high_value = 2.0,
                                  Real step_start = 0.5,
                                  Real step_end = 1.0) {
        auto step_func = [=](Real x) -> Real {
        ...
        initialize_field(step_func);
    }
    
    // Initialize with sinusoidal function
    void initialize_sinusoidal(Real amplitude = 0.5, 
                               Real frequency = 1.0,
                               Real mean_value = 1.0) {
        auto sine_func = [=](Real x) -> Real {
            return mean_value + amplitude * std::sin(2.0 * M_PI * frequency * x);
        };
        
        initialize_field(sine_func);
    }
    
    // Initialize with Gaussian function
    void initialize_gaussian(Real amplitude = 1.0,
                             Real center = 1.0,
                             Real width = 0.1,
                             Real background = 1.0) {
        auto gaussian_func = [=](Real x) -> Real {
            Real dx = x - center;
            return background + amplitude * std::exp(-dx * dx / (2.0 * width * width));
        };
        
        initialize_field(gaussian_func);
    }
