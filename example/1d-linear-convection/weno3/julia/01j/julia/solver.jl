...
mutable struct Cfd
    ...
    function Cfd(config::Any, mesh::Mesh)
        ...
        # 时间推进器
        integrator = if config.rk_order == 1
            RK1Integrator((config=config, domain=domain, solution=solution, residual_calculator=residual_calculator, boundary_condition=boundary_condition))
        elseif config.rk_order == 2
            RK2Integrator((config=config, domain=domain, solution=solution, residual_calculator=residual_calculator, boundary_condition=boundary_condition))
        elseif config.rk_order == 3
            RK3Integrator((config=config, domain=domain, solution=solution, residual_calculator=residual_calculator, boundary_condition=boundary_condition))
        else
            error("不支持的 RK 阶数: $(config.rk_order)")
        end
