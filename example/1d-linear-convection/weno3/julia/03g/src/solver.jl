...
include("reconstructor/weno5.jl") 
...
mutable struct Cfd
	...
	function Cfd(config::Any, mesh::Mesh)
		...
		reconstructor = ReconstructorFactory.create(full_cfd)
		
		full_cfd = merge(full_cfd, (; solution = solution, reconstructor = reconstructor))

		
		# 1. 创建 Equation 和 Problem
		equation = create_equation(config)
		problem = create_problem(config)		
		
		# 2. 初始上下文（仅包含不依赖其他组件的字段
		full_cfd = merge(full_cfd, (;equation = equation))
		full_cfd = merge(full_cfd, (;problem = problem))
		
		# 3. 创建边界条件（通过 problem）
		boundary_condition = create_boundary_condition(problem, full_cfd)
		full_cfd = merge(full_cfd, (boundary_condition = boundary_condition,))
		

		# 4. 创建 residual_calculator（现在可访问 equation）
		...
		# 7. 初始化解（通过 problem）
		ic = create_initial_condition(problem, config)
		Main.apply(ic, solution)  # 注意：apply 在 Main		
...
# --- Equation / Problem 创建函数 ---
function create_equation(config::Any)
    eq_type = config.equation_type
    if eq_type == "linear_advection"
        return LinearAdvectionEquation(config)
    else
        error("Unknown equation type: $eq_type")
    end
end

function create_problem(config::Any)
    prob_type = config.problem_type
    if prob_type == "linear_advection"
        return LinearAdvectionProblem(config)
    else
        error("Unknown problem type: $prob_type")
    end
end