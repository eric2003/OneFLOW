...
function performEnoWenoAnalysisBAK()
    # 1. 初始化网格
    mesh = Mesh()
...
function performEnoWenoAnalysis()
	...
	# 新增 WENO5
	println("Running WENO5 solver...")	
	
	config_weno5 = CfdConfig()
	with_reconstruction(config_weno5, "weno", 5)
	config_weno5.dt = 0.0025
	config_weno5.rk_order = 2

	cfd_weno5 = Cfd(config_weno5, mesh)
	run!(cfd_weno5)
	...
	plot_comparison(
		plotter, 
		[cfd_eno3.result, cfd_weno3.result, cfd_weno5.result],
		save_path="eno_weno_comparison.png"
	)	

    return cfd_eno3, cfd_weno3, cfd_weno5
end
