...
mutable struct Cfd
    ...
    function Cfd(config::Any, mesh::Mesh)
		...
		# 创建通量计算器
		flux_calculator = FluxCalculatorFactory.create(cfd_context)
		...
		# 构建边界条件上下文
		bc_context = (config = config, domain = domain)

		# 使用工厂创建边界条件
		boundary_condition = BoundaryConditionFactory.create(bc_context)		
		...
		# 使用工厂创建时间推进器
		integrator = TimeIntegratorFactory.create(integrator_context)		
