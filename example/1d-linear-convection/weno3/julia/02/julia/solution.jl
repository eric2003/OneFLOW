...
"""
根据配置初始化场
注：暂用硬编码替代 InitialConditionFactory
"""
function initialize_from_config(sol::Solution, config::Any)
    ic_type = getfield_safe(config, :ic_type, "step")
    
    # 硬编码创建 IC（替代 InitialConditionFactory）
    ic = if ic_type == "step"
        StepFunctionIC(config)
    elseif ic_type == "sin"
        SineWaveIC(config)
    elseif ic_type == "gaussian"
        GaussianPulseIC(config)
    else
        error("未知初始条件类型: $ic_type")
    end
    
    apply(ic, sol)  # 调用 IC.apply
end
