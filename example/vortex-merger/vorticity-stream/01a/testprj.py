...
def vm_ic(nx,ny,x,y,w):
    ...
    for j in range(1, ny+2):
        for i in range(1, nx+2):    
            c1 = np.exp(-sigma*((x[i-1]-xc1)**2 + (y[j-1]-yc1)**2))
            c2 = np.exp(-sigma*((x[i-1]-xc2)**2 + (y[j-1]-yc2)**2))
            w[i,j] = np.exp(-sigma*((x[i-1]-xc1)**2 + (y[j-1]-yc1)**2)) + \
                     np.exp(-sigma*((x[i-1]-xc2)**2 + (y[j-1]-yc2)**2))
            print("i,j=",i," ",j," c1 = ",c1," c2 = ",c2)
...
def dump_fieldTEST(x,y,field,nx,ny,filename):
    file = open(filename, "w")
    file.write("{0} {1}\n" .format(nx,ny));
    for j in range(0, ny+1):
        for i in range(0, nx+1):
            file.write("{0} {1} {2} {3} {4}\n" .format(i,j,x[i],y[j],field[i,j]));
    file.close()    
...
for j in range(0, ny+1):
    for i in range(0, nx+1):
        un0[i,j] = wn[i+1,j+1]
        print("i,j=",i," ",j,un0[i,j])
        
        
#dump_field(x,y,un0,nx,ny,"vm0.txt")
dump_fieldTEST(x,y,un0,nx,ny,"vm0.txt")
