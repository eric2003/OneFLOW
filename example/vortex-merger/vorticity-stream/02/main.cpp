...
void dumpfieldTEST( VectorXd & x, VectorXd & y, MatrixXd & field, int nx, int ny, const std::string & filename );
...
void vm_icTEST( int nx, int ny, VectorXd & x, VectorXd & y, MatrixXd & w );
...
void dumpfieldTEST( VectorXd&x, VectorXd&y,  MatrixXd &field, int nx, int ny, const std::string &filename )
{
    ...
    std::format_to( std::ostream_iterator<char>( file ), "{0} {1}\n", nx, ny );
    for ( int j = 0; j < ny + 1; ++ j )
    {
        for ( int i = 0; i < nx + 1; ++ i )
        {
            std::format_to(std::ostream_iterator<char>(file), "{0} {1} {2} {3} {4:e}\n",
                i, j, x(i), y(j), field(i,j) );
        }
    }
...
int main( int argc, char **argv )
{
    ...
    //dumpfieldTEST( x, y, un0, nx, ny, "vm0.txt" );
