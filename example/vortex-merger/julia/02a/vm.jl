...
function cal_stream_function(nx,ny,dx,dy,sf,vt)
    # parameters for SOR iteration
	#w = Array{Float64}(undef, nx+2, ny+2)
	w = zeros(Float64, nx+2, ny+2)
    MaxIt=1400;
    beta=1.5;
    MaxErr=1.0e-6;
    ...
    for iter = 0:MaxIt-1
        ...
        rerr = err / err0
        #println("iter=",iter," rerr=",rerr," err = ",err, " err0 = ",err0)
        #stop if iteration has converged
        #if ( err < MaxErr )
		#if ( rerr <= 0.0001 || err < MaxErr )
		if ( err < MaxErr )
            break
		end
	end
...
function rhsnew(nx,ny,dx,dy,re,w,r)

    # compute streamfunction from vorticity
    #s = Array{Float64}(undef, nx+2, ny+2)
	s = zeros(Float64, nx+2, ny+2)
    f = Array{Float64}(undef, nx, ny)
    f = -w[2:nx+1,2:ny+1]
