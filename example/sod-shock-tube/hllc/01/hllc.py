...
class MyPlot:
    ...
    def PlotCompare( self, x, q ):
        ...
        sizes = 4
        plt.figure("Sod's shock-tube problem+HLLC Scheme+WENO-5 reconstruction", figsize=(10, 8), dpi=100)
        plt.subplot(2, 2, 1)
        ...
        plt.scatter(x, uu,  marker= "o", s=sizes, facecolors='none', edgecolors='blue', label="OneFLOW-CFD" )
        plt.xlabel("$x$")
        plt.ylabel(r"$u$")
        plt.legend()
        ...
        plt.scatter(x, ee,  marker= "o", s=sizes, facecolors='none', edgecolors='blue', label="OneFLOW-CFD" )
        plt.xlabel("$x$")
        plt.ylabel(r"$E$")
        plt.legend()
        ...
        plt.scatter(x, pp, marker= "o", s=sizes, facecolors='none', edgecolors='blue', label="OneFLOW-CFD" )
        plt.xlabel("$x$")
        plt.ylabel(r"$p$")
        plt.legend()
...
# Riemann solver: HLLC
#-----------------------------------------------------------------------------#
def hllc( nx, gamma, qL, qR, fL, fR, f ):
    Ds = np.zeros( 3 )
    gm = gamma - 1.0
    ...
    for i in range(0, nx+1):
        ...
        term1 = pR - pL + rhoL*uL*(SL-uL) - rhoR*uR*(SR-uR)
        term2 = rhoL*(SL-uL) - rhoR*(SR-uR)
        SP = term1 / term2 #never get zero
