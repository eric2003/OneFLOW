...
{
    //logFile << "UploadInterfaceField\n";
    for ( int iZone = 0; iZone < LocalZone::nZones; ++ iZone )
    {
        //logFile << "ilocal zone = " << iZone << " global zone = " << LocalZone::global_zoneids[ iZone ] << "\n";
        Interface * interface = Global::interfaces[ iZone ];

        std::vector<int> & send_to_zones = interface->send_to_zones;
        int nsend_zones = send_to_zones.size();

        //logFile << "nsend_zones = " << nsend_zones << "\n";
        ...
        for ( int iSend = 0; iSend < nsend_zones; ++ iSend )
        {
            ...
            //logFile << "iSend = " << iSend << " zone_to_send " << zone_to_send << "\n";
            //logFile << "donorfaces = ";
            //for ( int i = 0; i < donorfaces.size(); ++ i )
            //{
            //    logFile << donorfaces[ i ] << " ";
            //}
            //logFile << "\n";
            //logFile << "donordatas = ";
            //for ( int i = 0; i < donordatas.size(); ++ i )
            //{
            //    logFile << donordatas[ i ] << " ";
            //}
            //logFile << "\n";
            ...
            for ( int i = 0; i < nface; ++ i )
            {
                ...
                //logFile << "i = " << i << " nface " << nface << " i_donor_cell = " << i_donor_cell << "\n";
                donordatas[ i ] = field->u[ i_donor_cell ];
            }

            //logFile << "donordatas after = ";
            //for ( int i = 0; i < donordatas.size(); ++ i )
            //{
            //    logFile << donordatas[ i ] << " ";
            //}
...
void Solver::UpdateInterfaceField()
{
    ...
    {
        //logFile << "UpdateInterfaceField iZone = " << iZone << "\n";
        ...
        for ( int iNei = 0; iNei < ndonor_zones; ++ iNei )
        {
            ...
            if ( Parallel::pid == send_pid )
            {
                ...
                //donorface = interface->donorfaces_for_send[ iNei ];
            ...
            if ( Parallel::pid == recv_pid )
            {
                ...
                //donorface.resize( nsend );
            }
            HXSendRecvData( donordata.data(), donordata.size(), send_pid, recv_pid );
            //HXSendRecvData( donorface.data(), donorface.size(), send_pid, recv_pid );
            //logFile << "donordata = ";
            ...
            //logFile << "\n";
            //logFile << "donorface = ";
            //for ( int i = 0; i < donorface.size(); ++ i )
            //{
            //    logFile << donorface[ i ] << " ";
            //}
            //logFile << "\n";
...
void Solver::DownloadInterfaceField()
{
    for ( int iZone = 0; iZone < LocalZone::nZones; ++ iZone )
    {
        ...
        //logFile << "DownloadInterfaceField\n";
        //logFile << "LocalZone::nZones = " << LocalZone::nZones << "\n";
        //logFile << "iZone = " << iZone << " LocalZone::global_zoneids[iZone] = " << LocalZone::global_zoneids[ iZone ] << "\n";

        //logFile << "field->u[ 1 ] = " <<field->u[ 1 ] << "\n";
        //logFile << "field->u[ 21 ] = " <<field->u[ 21 ] << "\n";
        ...
        for ( int iFace = 0; iFace < nInterFaces; ++ iFace )
        {
            ...
            //logFile << "donor_value = " << donor_value << "\n";
...
void Post::ReorderZones()
{
    ...
    {
        std::cout << "Parallel::pid = " << Parallel::pid << " ";
        for ( int i = 0; i < xmin_list.size(); ++ i )
        {
            std::cout << "(" << xmin_list[ i ] << "," << xmax_list[ i ] << ") ";
        }
        std::cout << "\n";
    }
...
void Post::GatherField()
{
    ...
    for ( int iZone = 0; iZone < ZoneState::nZones; ++ iZone )
    {
        ...
        logFile << "iZone = " << iZone << " zoneid = " << zoneid << "\n";
        ...
        logFile << "Parallel::pid = " << Parallel::pid << " ZoneState::pids[ zoneid ] = " << ZoneState::pids[ zoneid ] << "\n";
    ...
    logFile << "Post::GatherField  3 \n";
}

void Post::DumpField()
{
    //logFile << "Post::DumpField() \n";
    if ( !Parallel::IsServer() ) return;
    int ni_total = this->x.size();
    //logFile << " DumpField x.size() = " << x.size() << "\n";
