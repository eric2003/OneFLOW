...
for k in range(1, nt+1):
    ...
    for i in range(1, nx):
        a[i] = 2.0  / ( alpha * dt ) - 12.0  /( dx * dx )
        b[i] = 20.0 / ( alpha * dt ) + 24.0 / ( dx * dx )
        c[i] = 2.0  / ( alpha * dt ) - 12.0  /( dx * dx )
        
        aa   = 2.0  / ( alpha * dt ) + 12.0  /( dx * dx )
        bb   = 20.0 / ( alpha * dt ) - 24.0 / ( dx * dx )
        cc   = 2.0  / ( alpha * dt ) + 12.0  /( dx * dx )    
