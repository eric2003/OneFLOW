...
class Mgdata:
    ...
    def allocateField( self, vh, fh ):
        ...
        self.r4h = np.zeros((self.nx[2]+1, self.ny[2]+1), dtype=np.float64)
        self.e4h = np.zeros((self.nx[2]+1, self.ny[2]+1), dtype=np.float64)
        
        self.f8h = np.zeros((self.nx[3]+1, self.ny[3]+1), dtype=np.float64)
        self.v8h = np.zeros((self.nx[3]+1, self.ny[3]+1), dtype=np.float64)
        
        self.r8h = np.zeros((self.nx[3]+1, self.ny[3]+1), dtype=np.float64)
        self.e8h = np.zeros((self.nx[3]+1, self.ny[3]+1), dtype=np.float64)
        
        self.f16h = np.zeros((self.nx[4]+1, self.ny[4]+1), dtype=np.float64)
        self.v16h = np.zeros((self.nx[4]+1, self.ny[4]+1), dtype=np.float64)
    ...
    def Multigrid( self ):
        ...
        #calc r4h
        compute_residual( self.nx[2], self.ny[2], self.dx[2], self.dy[2], self.f4h, self.v4h, self.r4h )
        
        #restrict r4h to f8h
        restriction(self.nx[2], self.ny[2], self.nx[3], self.ny[3], self.r4h, self.f8h)
        
        #zero v8h
        self.v8h = np.zeros((self.nx[3]+1, self.ny[3]+1), dtype=np.float64)
        
        #solve v8h
        gauss_seidel_mg(self.nx[3], self.ny[3], self.dx[3], self.dy[3], self.f8h, self.v8h, self.v3)
        
        #calc r8h
        compute_residual( self.nx[3], self.ny[3], self.dx[3], self.dy[3], self.f8h, self.v8h, self.r8h )
        
        #restrict r8h to f16h
        restriction(self.nx[3], self.ny[3], self.nx[4], self.ny[4], self.r8h, self.f16h)
        
        #zero v16h
        self.v16h = np.zeros((self.nx[4]+1, self.ny[4]+1), dtype=np.float64)
        
        #solve v16h
        gauss_seidel_mg(self.nx[4], self.ny[4], self.dx[4], self.dy[4], self.f16h, self.v16h, self.v3)

        #interpolate e8h from v16h
        prolongation( self.nx[4], self.ny[4], self.nx[3], self.ny[3], self.v16h, self.e8h )
        
        # correct v8h
        for j in range(1, self.ny[3]):
            for i in range(1, self.nx[3]):
                self.v8h[i,j] = self.v8h[i,j] + self.e8h[i,j]        
        
        #interpolate e4h from v8h
        prolongation( self.nx[3], self.ny[3], self.nx[2], self.ny[2], self.v8h, self.e4h )

        # correct v4h
        for j in range(1, self.ny[2]):
            for i in range(1, self.nx[2]):
                self.v4h[i,j] = self.v4h[i,j] + self.e4h[i,j]
