...
def plot_mixed_line( xst, xed ):
    ds = xed - xst
    ds1 = ds / 4
    x1 = xst + ds1
    x2 = xed - ds1
    x = np.array([xst, x1, x2, xed])  # 按照1/4, 1/2, 1/4的比例分割
    plt.plot([x[0], x[1]], [0, 0], 'k-', linewidth=1)  # 第一段实线
    plt.plot([x[1], x[2]], [0, 0], 'k--', linewidth=1) # 第二段虚线
    plt.plot([x[2], x[3]], [0, 0], 'k-', linewidth=1)  # 第三段实线
    return
...
def plot_label(y0, xv, lr, txt_name):
    ...
    #plt.text(xv+0.3, y0+0.5, r'$u_{i+\frac{1}{2}}^R$', fontsize=12, ha='center')
    lrname = r'$u_{i+\frac{1}{2}}^' + lr + r'$'
    plt.text(xv-0.3, y0+0.5, lrname, fontsize=12, ha='center')
...
lr = 'L'
txt_name = '(a) Left-side reconstruction'

plot_rect(x,y,width,height,rectangle_color)
plot_label(y0,xv,lr,txt_name)
