...
class Mesh(BaseMesh):
    ...
    def __init__(self, nghosts=2,ishift=0, ym=0, nodebased=0,periodic=False): # Added: periodic parameter, default False
        ...
        if nodebased == 1:
            nmc = 2
        self.ncells = 2*self.nghosts+nmc # Number of cells in main mesh
    ...
    def plot_node_mesh(self):
        ...
        for i in range(self.nnodes):
            ...
            if abs(self.nnodes - i) <= 3:
                txt = self.get_dollar_label("N",i+1-self.nnodes)
            plt.text(xm, ym-dym, txt, fontsize=12, ha='center')
...
def plot_cfd_figure(periodic=False): # Added: periodic parameter, default False
    ...
    nghost3 = 3
    mesh = Mesh(nghost3,-1, nodebased=1,periodic=periodic) # Pass periodic
    ...
    filename = 'mesh_node1d-v2.png' if periodic else 'cfd.png'
