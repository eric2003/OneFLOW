...
void SplitBc::CreateInterfaceBcFromParentBcNewVersion( SplitBc * parentSplitBc )
{
    ...
    //targetZone->CreateChildInterfaceBc( targetRegionBox, this->splitZone );
...
void SplitZone::CreateChildInterfaceBc( RangeRegion &targeRegionBox, SplitZone * sourceZone )
{
    std::vector< SplitBc * > & splitBcList = this->GetSplitBcList();
    ...
    for ( int iSplitBc = 0; iSplitBc < nSplitBcs; ++ iSplitBc )
    {
        ...
        if ( this->CreateChildInterfaceBc( splitBc, targeRegionBox, sourceZone ) ) return;
    }
}

bool SplitZone::CreateChildInterfaceBc( SplitBc * interfaceBc, RangeRegion & targeRegionBox, SplitZone * sourceZone )
{
    RangeRegion overlapRegion;
    bool overlap_flag = ComputeOverlapRegion( interfaceBc->region, targeRegionBox, overlapRegion );
    if ( ! overlap_flag ) return false;
    ...
    std::vector< SplitBc * > * child = interfaceBc->child;

    if ( interfaceBc->child )
    {
        for ( int iChild = 0; iChild < interfaceBc->child->size(); ++ iChild )
        {
            SplitBc * childSplitBc = ( * interfaceBc->child )[ iChild ];
            if ( this->CreateChildInterfaceBc( childSplitBc, targeRegionBox, sourceZone ) ) return true;
        }
    }
    else
    {
        interfaceBc->child = new std::vector< SplitBc * >();
    }
    ...
    childInterfaceBc->SetRegion( overlapRegion );
    childInterfaceBc->bcType = interfaceBc->bcType;
    childInterfaceBc->splitZone = interfaceBc->splitZone;


    interfaceBc->child->push_back( childInterfaceBc );
...
SplitZone * Partition::Split( SplitZone * zone, double nCell )
{
    ...
    zoneL->SetParent( zone );

    SplitZone * zoneR = new SplitZone();
    zoneR->SetParent( zone );
...
void Partition::DumpPartitionedGridFile()
{
    ...
    for ( int iProc = 0; iProc < nProc; ++ iProc )
    {
        ...
        for ( int iZone = 0; iZone < nZones; ++ iZone )
        {
            ...
            //int nBcLists = splitZone->splitBcList.size();
            std::vector< SplitBc * > leafBcList;
            splitZone->GetLeafBcList( leafBcList );
            int nLeafBcList = leafBcList.size();

            std::vector<SplitBc * > bccoList;
            std::vector<SplitBc * > bc1to1List;

            for ( int ibc = 0; ibc < nLeafBcList; ++ ibc )
            {
                SplitBc * splitBc = leafBcList[ ibc ];
                if ( splitBc->bcType == BCTypeUserDefined )
                {
                    bc1to1List.push_back( splitBc );
                }
            ...
            for ( int i1to1 = 0; i1to1 < n1to1; ++ i1to1 )
            {
                ...
                int id = splitBc->patch->splitZone->newIndex;
                int oldZoneIndex = splitBc->patch->splitZone->zoneIndex;
                std::cout << "id = " << id << "\n";
