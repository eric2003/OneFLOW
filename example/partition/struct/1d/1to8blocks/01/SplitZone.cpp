...
void RangeRegion::ToGlobal( std::vector<int> & oriPoint )
{
    int nSize = this->start.size();
    for ( int m = 0; m < nSize; ++ m )
    {
...
void RangeRegion::ToLocal( std::vector<int> & oriPoint )
{
    int nSize = this->start.size();
    for ( int m = 0; m < nSize; ++ m )
    {
...
void InterfaceSplitBc::GetLeafInterfaceBc( std::vector< InterfaceSplitBc * > & leafInterfaceBcList )
{
    int nChild = this->child.size();
    if ( nChild == 0 )
    {
        leafInterfaceBcList.push_back( this );
    }
    else
    {
        for ( int iChild = 0; iChild < nChild; ++ iChild )
        {
            InterfaceSplitBc * interfaceSplitBc = this->child[ iChild ];
            interfaceSplitBc->GetLeafInterfaceBc( leafInterfaceBcList );
        }
    }
}
...
void SplitZone::GetLeafInterfaceBc( std::vector< InterfaceSplitBc * > & leafInterfaceBcList )
{
    int nInterfaceBcs = this->interfaceSplitBcList.size();
    for ( int i = 0; i < nInterfaceBcs; ++ i )
    {
        InterfaceSplitBc * interfaceSplitBc = this->interfaceSplitBcList[ i ];
        interfaceSplitBc->GetLeafInterfaceBc( leafInterfaceBcList );
    }
}
...
void SplitZone::GetChildDonorRegion( RangeRegion & subDonorRegion, InterfaceInfo & interfaceInfo )
{
    ...
    RangeRegion normalizedDonorRegion = subDonorRegion;
    normalizedDonorRegion.Normalize();
    int nChild = this->child.size();
    for ( int ichild = 0; ichild < nChild; ++ ichild )
    {
        ...
        for ( int iInter = 0; iInter < nInters; ++ iInter )
        {
            ...
            g_region.ToGlobal( sub_zone->dimInfo.oriPoint );
            if ( g_region == normalizedDonorRegion )
            {
                RangeRegion donorRegion = subDonorRegion;
                donorRegion.ToLocal( sub_zone->dimInfo.oriPoint );

                interfaceInfo.donor_zone = sub_zone;
                interfaceInfo.donor_interface = interfaceSplitBc;
                interfaceInfo.donor_region = donorRegion;
                return;
            }
        }
    }
