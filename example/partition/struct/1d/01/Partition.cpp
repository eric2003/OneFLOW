...
void Partition::DumpPartitionedGridFile()
{
    ...
    for ( int iProc = 0; iProc < nProc; ++ iProc )
    {
        ...
        for ( int iZone = 0; iZone < nZones; ++ iZone )
        {
            ...
            {
                isize[ i ] = splitZone->dimInfo.dimList[ i ];
            }
            for ( int i = 0; i < index_dim; ++ i )
            {
                int ii = index_dim * 1;
                isize[ ii + i ] = splitZone->dimInfo.dimList[ i ] - 1;
            }
            for ( int i = 0; i < index_dim; ++ i )
            {
                int ii = index_dim * 2;
            ...
            for ( int icoord = 0; icoord < ncoords; ++ icoord )
            {
                ...
                //cg_coord_write( fileId, baseId, zoneId, dataType, coordname.c_str(), const void * coord_ptr, &coorId );
                //    
                //    
