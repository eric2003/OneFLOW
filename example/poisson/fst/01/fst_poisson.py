...
def my_fst( nx, ny, f, fmn ):
    for n in range(0, ny):
        for m in range(0, nx):
            fmn[m,n] = 0.0
            print("my_fst m,n=",m,n)
            for j in range(0, ny):
                c1 = np.sin(np.pi*n*j/ny)
                for i in range(0, nx):
                    c2 = np.sin(np.pi*m*i/nx)
                    fmn[m,n] += f[i,j] * c1 * c2
    return

def inverse_fst( nx, ny, umn ):
    uij = np.zeros( ( nx, ny ) )
    coef = 4.0 / ( nx * ny )
    for j in range(0, ny):
        for i in range(0, nx):    
            uij[i,j] = 0.0
            print("inverse_fst i,j=",i,j)
            for n in range(0, ny):
                c1 = np.sin(np.pi*n*j/ny)
                for m in range(0, nx):            
                    c2 = np.sin(np.pi*m*i/nx)
