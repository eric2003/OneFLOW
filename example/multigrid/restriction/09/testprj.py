...
class MG:
    ...
    def allocate( self, nx, ny, dx, dy, level ):
        ...
        self.dx = dx
        self.dy = dy
        self.un = np.zeros( ( nx + 1, ny + 1 ), dtype=np.float64)
        self.f  = np.zeros( ( nx + 1, ny + 1 ), dtype=np.float64)
...
class Mgdata:
    ...
    def allocateField( self, un, f ):
        self.prol_fine = np.zeros((self.nx[0]+1, self.ny[0]+1), dtype=np.float64)
        self.r = np.zeros((self.nx[0]+1, self.ny[0]+1), dtype=np.float64)
        self.un = un.copy()
        self.f = f.copy()
...
def MultigridBAK( mg, f, un, r, prol_fine, fc ):
    # call relaxation on fine grid and compute the numerical solution
    # for fixed number of iteration
    gauss_seidel_mg(mg.nx[0], mg.ny[0], mg.dx[0], mg.dy[0], f, un, mg.v1) 
    ...
    gauss_seidel_mg(mg.nx[1], mg.ny[1], mg.dx[1], mg.dy[1], fc, unc, mg.v3)
    ...
    # relax v2 times
    gauss_seidel_mg(mg.nx[0], mg.ny[0], mg.dx[0], mg.dy[0], f, un, mg.v2)
    
def Multigrid( mg, fc ):
    # call relaxation on fine grid and compute the numerical solution
    # for fixed number of iteration
    gauss_seidel_mg(mg.nx[0], mg.ny[0], mg.dx[0], mg.dy[0], mg.f, mg.un, mg.v1) 
    
    # check for convergence only for finest grid
    # compute the residual and L2 norm
    compute_residual( mg.nx[0], mg.ny[0], mg.dx[0], mg.dy[0], mg.f, mg.un, mg.r )
    
    # restrict the residual from fine level to coarse level
    restriction(mg.nx[0], mg.ny[0], mg.nx[1], mg.ny[1], mg.r, fc)
    ...
    # prolongate solution from coarse level to fine level
    prolongation( mg.nx[1], mg.ny[1], mg.nx[0], mg.ny[0], unc, mg.prol_fine )
    
    # correct the solution on fine level
    for j in range(1, mg.ny[0]):
        for i in range(1, mg.nx[0]):
            mg.un[i,j] = mg.un[i,j] + mg.prol_fine[i,j]

    # relax v2 times
    gauss_seidel_mg(mg.nx[0], mg.ny[0], mg.dx[0], mg.dy[0], mg.f, mg.un, mg.v2)
    
def MultigridCycle(mg, nx, ny, dx, dy, un, f, r, output):
    tolerance = 1.0e-12
    max_iter = 100000
    #nlevel = 2
    #mg = Mgdata()
    ...
    mh = MG()
    mh.allocate( nx, ny, dx, dy, 0 )
    
    m2h = MG()
    m2h.allocate( mg.nx[1], mg.ny[1], mg.dx[1], mg.dy[1], 1 )
    ...
    while True:
        #Multigrid( mg, f, un, r, prol_fine, fc )
        Multigrid( mg, fc )
        # compute the l2norm of residual
        rms = compute_l2norm(mg.nx[0], mg.ny[0], mg.r)
...
res = MultigridCycle(mg, nx, ny, dx, dy, un, f, r, output )
...
cntr = ax.contourf(x, y, mg.un, levels=20, cmap="jet")
