...
def Multigrid( mg ):
    ...
    restriction(mg.nx[0], mg.ny[0], mg.nx[1], mg.ny[1], mg.r, mg.fc)
    
    # set solution zero on coarse grid
    mg.unc = np.zeros((mg.nx[1]+1, mg.ny[1]+1), dtype=np.float64)

    # solve on the coarsest level and relax V3 times
    gauss_seidel_mg(mg.nx[1], mg.ny[1], mg.dx[1], mg.dy[1], mg.fc, mg.unc, mg.v3)
    
    # prolongate solution from coarse level to fine level
    prolongation( mg.nx[1], mg.ny[1], mg.nx[0], mg.ny[0], mg.unc, mg.prol_fine )
