...
class Mgdata:
    ...
    def allocateField( self, un, f ):
        ...
        self.fc  = np.zeros((self.nx[1]+1, self.ny[1]+1), dtype=np.float64)
        self.unc = np.zeros((self.nx[1]+1, self.ny[1]+1), dtype=np.float64)
        
    def Multigrid( self ):
        # call relaxation on fine grid and compute the numerical solution
        # for fixed number of iteration
        gauss_seidel_mg(self.nx[0], self.ny[0], self.dx[0], self.dy[0], self.f, self.un, self.v1) 
        
        # check for convergence only for finest grid
        # compute the residual and L2 norm
        compute_residual( self.nx[0], self.ny[0], self.dx[0], self.dy[0], self.f, self.un, self.r )
        
        # restrict the residual from fine level to coarse level
        restriction(self.nx[0], self.ny[0], self.nx[1], self.ny[1], self.r, self.fc)
        
        # set solution zero on coarse grid
        self.unc = np.zeros((self.nx[1]+1, self.ny[1]+1), dtype=np.float64)
    
        # solve on the coarsest level and relax V3 times
        gauss_seidel_mg(self.nx[1], self.ny[1], self.dx[1], self.dy[1], self.fc, self.unc, self.v3)
        
        # prolongate solution from coarse level to fine level
        prolongation( self.nx[1], self.ny[1], self.nx[0], self.ny[0], self.unc, self.prol_fine )
        
        # correct the solution on fine level
        for j in range(1, self.ny[0]):
            for i in range(1, self.nx[0]):
                self.un[i,j] = self.un[i,j] + self.prol_fine[i,j]
    
        # relax v2 times
        gauss_seidel_mg(self.nx[0], self.ny[0], self.dx[0], self.dy[0], self.f, self.un, self.v2)
        
    def init_norm( self ):
        compute_residual(self.nx[0], self.ny[0], self.dx[0], self.dy[0], self.f, self.un, self.r)
        rms = compute_l2norm(self.nx[0], self.ny[0], self.r)
        return rms
