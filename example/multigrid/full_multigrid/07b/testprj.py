...
def FMG_VCycle_Recursive_Level_4_VERSION(vh, fh, n, omega):
    ...
    e16h = np.zeros( int(n/16) + 1 )
    e16h = FMG_VCycle( e16h, rr[4], int(n/16), level+4, nCoarse, omega, vIter )
    
    e8h = Interpolate( e16h, int(n/16) )
    e8h = FMG_VCycle( e8h, rr[3], int(n/8), level+3, nCoarse, omega, vIter )
    
    e4h = Interpolate( e8h, int(n/8) )
    e4h = FMG_VCycle( e4h, rr[2], int(n/4), level+2, nCoarse, omega, vIter )
    
    e2h = Interpolate( e4h, int(n/4) )
    e2h = FMG_VCycle( e2h, rr[1], int(n/2), level+1, nCoarse, omega, vIter )
    
    eh = Interpolate( e2h, int(n/2) )
    eh = FMG_VCycle( eh, rr[0], int(n), level, nCoarse, omega, vIter )
