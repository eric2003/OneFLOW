...
#else
bool cpu_self_test(){constexpr int n=31;std::vector<double>q(3*n);for(int i=0;i<n;++i){q[i]=1.25;q[n+i]=.375;q[2*n+i]=.9/(G-1.0)+.5*1.25*.09;}oneflow_1d::EulerTrace t;oneflow_1d::OneDCpuEulerStep(q.data(),n,G,.05/n,1.0/n,oneflow_1d::EulerBoundary::Periodic,t);return physical(t.state,n);}
#endif
}
int main(){std::cout<<std::setprecision(17)<<std::scientific;
#ifdef ONEFLOW_1D_USE_HIP
int n=257;bool a=run("smooth-periodic",smooth(n),smooth(n),n,10,oneflow_1d::EulerBoundary::Periodic);bool b=run("sod-transmissive",sod(n),sod(n),n,10,oneflow_1d::EulerBoundary::Transmissive);std::cout<<"OneFLOW 1D compressible Euler CPU/HIP validation: "<<((a&&b)?"PASS":"FAIL")<<"\n";return a&&b?0:1;
#else
bool ok=cpu_self_test();std::cout<<"OneFLOW 1D Euler CPU self-test: "<<(ok?"PASS":"FAIL")<<"\n";return ok?0:1;
#endif
}
